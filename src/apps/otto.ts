// Otto — snapshot of Claude Voice Assistant (pre-Ava rebrand, 2026-02-23)
...
export const OTTO_FILLER_PHRASES = [
...
export const OTTO_GOODBYE_PHRASES = [
...
export class OttoAssistant implements VoxnosApp {
  id = 'otto';
  name = 'Otto Voice Assistant';
  fillerPhrases = OTTO_FILLER_PHRASES;
  ...
  async onSpeech(context: AppContext, input: SpeechInput): Promise<AppResponse> {
    ...
    if (this.isGoodbye(userMessage)) {
      const goodbye = OTTO_GOODBYE_PHRASES[Math.floor(Math.random() * OTTO_GOODBYE_PHRASES.length)];
      return {
        speech: { text: goodbye },
        hangup: true,
      };
  ...
  async *streamSpeech(context: AppContext, input: SpeechInput): AsyncGenerator<StreamChunk, void, undefined> {
    ...
    if (this.isGoodbye(userMessage)) {
      const idx = Math.floor(Math.random() * OTTO_GOODBYE_PHRASES.length);
      yield { text: OTTO_GOODBYE_PHRASES[idx], hangup: true, cacheKey: `goodbye-${idx}` };
      return;
    }
    ...
    for await (const sentence of this.streamClaude(context, messages)) {
      const fillerIdx = OTTO_FILLER_PHRASES.indexOf(sentence);
      if (fillerIdx >= 0) {
            ...
            case 'content_block_start':
              if (event.content_block?.type === 'tool_use') {
                ...
                if (yieldedSentences.length === 0) {
                  const filler = OTTO_FILLER_PHRASES[Math.floor(Math.random() * OTTO_FILLER_PHRASES.length)];
                  yield filler;
